  ...
  <div>
       ...
       <el-dialog title="详情" :visible.sync="dialogFormMark" class="city_list" width="800px" style="margin-top:-100px">
          <table border="0" cellpadding="7" cellspacing="10" bgcolor="#f7f7f7" width="760" style="margin-top:-10px">
              ...
              <tr>
                  <td>其他费用：</td>
                  <td v-for="(item,index) in orderForm.favourable" v-if="index==0"><span class="table_span">{{item.mark}} {{item.price}}</span></td>
              </tr>
              <tr>
                  <td>整体优惠：</td>
                  <td v-for="(item,index) in orderForm.favourable" v-if="index==1"><span class="table_span">{{item.mark}} {{item.price}}</span></td>
              </tr>
              <tr>
                  <td>订单总额：</td>
